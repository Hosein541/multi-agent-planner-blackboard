...
class ChatIntent(BaseModel):
    intent: str = Field(
        ...
        - ask_explanation: When user asks for explanation about current planning or reports
        - general_question: For casual conversation or other questions"""
    )
    key: Optional[str] = Field(
        description="Only used when intent is 'update_profile'. Valid keys: 'task', 'fixed events', 'constraints', 'available hours', 'goals'",
...
def handle_chat_message(
    ...
    """You are an intelligent intent classifier for a Personal AI Planner.
    ...
    1. **update_profile** → Use when user wants to change their  information.
       - Examples: "my new task is going to gym", "set golas to achieve more money", "change available hours to 9:00 am - 7:00 pm", "constraints: No heavy work after 7 PM"

    2. **ask_explanation** → Use when user asks why, how, or explanation about current recommendation.
       - Examples: "why did you choose this plan?", "explain more about your decision", "what does this mean?"
    ...
    if intent.intent == "update_profile" and intent.key:
        ...
        general_prompt = ChatPromptTemplate.from_template(
            "You are a professional and friendly Personal Planning Advisor. Answer the user naturally.\n\nUser: {message}"
        )
